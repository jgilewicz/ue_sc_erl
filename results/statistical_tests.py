...
from process_results import load_environment_data, METHOD_LABELS
    ...
    for method in merged_data:
        ...
        for seed, df in merged_data[method].items():
            if 'total_steps' not in df.columns or y_metric not in df.columns:
                continue
            # Keep only the last 10% of training (total_steps >= 900,000)
            df_last_10 = df[df['total_steps'] >= 900000].dropna(subset=[y_metric])
...
def run_significance_tests(env_id, stable_values, base_dir="results"):
    ...
    # Save CSV
    csv_out = os.path.join(base_dir, f"{env_id}_significance_table.csv")
    df_tests.to_csv(csv_out, index=False)
    
    # Save LaTeX Table (GECCO publication ready)
    latex_out = os.path.join(base_dir, f"{env_id}_significance_table.tex")
    # Format p-value float column beautifully in LaTeX
    df_latex = df_tests.copy()
    df_latex['Stat p-value'] = df_latex['Stat p-value'].apply(lambda x: f"{x:.4e}" if x < 1e-3 else f"{x:.4f}")
    
    with open(latex_out, 'w') as f:
        f.write(df_latex.to_latex(index=False, column_format='lcccc', caption=f"Statistical Significance testing for {env_id} based on Shapiro-Wilk normality checking and subsequent Welch's t-test / Mann-Whitney U test.", label=f"tab:sig_{env_id}"))
        
    # Save Markdown for walkthrough / visual checking
    md_out = os.path.join(base_dir, f"{env_id}_significance_table.md")
    with open(md_out, 'w') as f:
        f.write(f"# Statistical Significance Testing - {env_id}\n\n")
        f.write("Normality check via Shapiro-Wilk (alpha=0.05). Comparison uses Welch's t-test (if normal) or Mann-Whitney U (if non-normal).\n\n")
